...
class BranchRenderer extends Component<BranchRendererProps> {
    render() {
        const { data: { children, name }, updateTreeToDb } = this.props;
        return (
            <div style={{ margin: 20, marginLeft: 50, border: "5px solid red" , padding: 5}}>
                ...
                {children.map((currentNode: any, index:number) => {
                    if (currentNode.children) {
                        return <BranchRenderer data={currentNode} key={index} updateTreeToDb={updateTreeToDb}/>
                    } else {
                        return <Leaf data={currentNode} key={index} updateTreeToDb={updateTreeToDb}/>
                    }
                })
                }
            </div>
